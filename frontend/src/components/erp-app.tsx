...
import {
  BarChart3, Bell, BookOpenCheck, Boxes, Building2, CalendarDays, ChevronDown, CircleDollarSign,
...
export function ErpApp() {
  ...
  async function bootstrap() {
    ...
    try {
      const status = await api<{ requiresSetup: boolean }>("/auth/setup-status");
      setRequiresSetup(status.requiresSetup);
      if (getToken() && !status.requiresSetup) {
        try { setUser(await api<CurrentUser>("/auth/me")); }
        catch { setToken(null); setUser(null); }
      }
    } catch (caught) { setStartupError(caught instanceof Error ? caught.message : "Unable to connect to the ERP API."); }
    finally { setLoading(false); }
  }
  useEffect(() => {
    api<{ requiresSetup: boolean }>("/auth/setup-status")
      .then(async (status) => {
        setRequiresSetup(status.requiresSetup);
        if (getToken() && !status.requiresSetup) {
          try { setUser(await api<CurrentUser>("/auth/me")); }
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center bg-[#f3f5f2] text-sm text-slate-500">Preparing your workspace…</div>;
  if (startupError) return (
    <main className="flex min-h-screen items-center justify-center bg-[#f3f5f2] p-6"><div className="max-w-md rounded-2xl border border-slate-200 bg-white p-7 text-center"><Boxes className="mx-auto size-8 text-[#487064]" /><h1 className="mt-4 text-xl font-semibold">The ERP API is unavailable</h1><p className="mt-2 text-sm leading-6 text-slate-500">{startupError}</p><button onClick={() => void bootstrap()} className="primary-button mt-6">Try again</button></div></main>
  );
  if (!user) return <AuthScreen requiresSetup={requiresSetup} onAuthenticated={setUser} />;
...
function AppShell({ user, onLogout }: { user: CurrentUser; onLogout: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-[#f3f5f2] text-slate-900 lg:grid lg:grid-cols-[264px_1fr]">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[264px] flex-col bg-[#102d2b] text-white transition-transform lg:sticky lg:top-0 lg:h-screen ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="flex h-20 items-center justify-between border-b border-white/8 px-5">
          <div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl bg-[#d9f56f] text-[#102d2b]"><Building2 className="size-5" /></span><div><p className="font-semibold tracking-tight">Northstar ERP</p><p className="text-[11px] text-white/40">Enterprise suite</p></div></div>
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto px-3 py-5">
          ...
          <div className="space-y-1">
            {nav.map((item) => {
              ...
              return <button key={item.key} onClick={() => navigate(item.key)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition ${active === item.key ? "bg-white/11 font-semibold text-white" : "text-white/58 hover:bg-white/6 hover:text-white"}`}><Icon className={`size-[18px] ${active === item.key ? "text-[#d9f56f]" : ""}`} />{item.title}</button>;
            })}
          </div>
        </nav>
        <div className="border-t border-white/8 p-4">
          <div className="flex items-center gap-3 rounded-xl bg-white/6 p-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-[#d9f56f] text-sm font-bold text-[#173f3b]">{initials(user.fullName)}</span><div className="min-w-0"><p className="truncate text-sm font-medium">{user.fullName}</p><p className="truncate text-[11px] text-white/40">{user.role}</p></div></div>
        </div>
      </aside>

      <div className="min-w-0">
        <header className="sticky top-0 z-20 flex h-20 items-center justify-between border-b border-slate-200/80 bg-[#f3f5f2]/95 px-4 backdrop-blur sm:px-7 lg:px-9">
          ...
          <div className="relative flex items-center gap-2">
            ...
            <button onClick={() => setProfileOpen((value) => !value)} className="ml-1 flex items-center gap-2 rounded-xl border border-slate-200 bg-white py-1.5 pl-1.5 pr-2.5 text-left">
              <span className="flex size-8 items-center justify-center rounded-lg bg-[#173f3b] text-xs font-bold text-white">{initials(user.fullName)}</span><span className="hidden text-sm font-medium sm:block">{user.fullName.split(" ")[0]}</span><ChevronDown className="size-3.5 text-slate-400" />
            </button>

...
type Props = { requiresSetup: boolean; onAuthenticated: (user: CurrentUser) => void };

export function AuthScreen({ requiresSetup, onAuthenticated }: Props) {
  ...
  return (
    <main className="min-h-screen bg-[#f3f5f2] text-slate-950">
      <div className="mx-auto grid min-h-screen max-w-[1500px] lg:grid-cols-[1.08fr_.92fr]">
        <section className="relative hidden overflow-hidden bg-[#102d2b] px-14 py-12 text-white lg:flex lg:flex-col">
          ...
          <div className="relative flex items-center gap-3">
            <span className="flex size-11 items-center justify-center rounded-xl bg-[#d9f56f] text-[#102d2b]">
          ...
          <div className="relative my-auto max-w-xl py-16">
            <p className="mb-5 text-xs font-bold uppercase tracking-[.22em] text-[#d9f56f]">One operating system</p>
            ...
            <div className="mt-10 grid max-w-lg gap-3 sm:grid-cols-2">
              {["Claims-based access", "Live SQL reporting", "Complete audit trail", "No seeded records"].map((item) => (
                <div key={item} className="flex items-center gap-3 text-sm text-white/78">
                  <span className="flex size-6 items-center justify-center rounded-full bg-white/10"><Check className="size-3.5 text-[#d9f56f]" /></span>{item}
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="flex items-center justify-center px-6 py-12 sm:px-12">
          <div className="w-full max-w-md">
            <div className="mb-10 flex items-center gap-3 lg:hidden">
              <span className="flex size-10 items-center justify-center rounded-xl bg-[#102d2b] text-[#d9f56f]"><Building2 className="size-5" /></span>
              <span className="font-semibold">Northstar ERP</span>
            </div>
            <span className="mb-6 flex size-12 items-center justify-center rounded-2xl bg-[#e5eccd] text-[#294b42]"><ShieldCheck className="size-6" /></span>
            <p className="text-sm font-semibold text-[#487064]">{requiresSetup ? "Secure first-run setup" : "Welcome back"}</p>
            ...
            <form onSubmit={submit} className="mt-8 space-y-5">
              ...
              <button disabled={busy} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#173f3b] px-5 text-sm font-semibold text-white transition hover:bg-[#0f302d] disabled:cursor-not-allowed disabled:opacity-60">
                {busy ? "Please wait…" : requiresSetup ? "Create administrator" : "Sign in"}{!busy && <ArrowRight className="size-4" />}
              </button>
            </form>
          </div>
        </section>

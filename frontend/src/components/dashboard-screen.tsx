  ...
  return (
    <section>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map((metric) => (
          <article key={metric.label} className="rounded-2xl border border-slate-200/80 bg-white p-5">
            <div className="flex items-start justify-between"><p className="text-sm font-medium text-slate-500">{metric.label}</p><span className="flex size-9 items-center justify-center rounded-xl bg-[#e7f3fb] text-[#245a8d]"><metric.icon className="size-4.5" /></span></div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.3fr_.7fr]">
        <article className="rounded-2xl border border-slate-200/80 bg-[#102f55] p-6 text-white">
          <div className="flex items-center justify-between">
            ...
            <CircleDollarSign className="size-6 text-[#55d7f3]" />
          </div>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <div className="border-l-2 border-[#55d7f3] pl-4">
              <div className="flex items-center gap-2 text-xs text-white/50"><ArrowUpRight className="size-3.5 text-[#55d7f3]" />Sales order value</div>
          ...
          <div className="mt-8 h-2 overflow-hidden rounded-full bg-white/10">
            <div className="h-full rounded-full bg-[#55d7f3]" style={{ width: `${ratio(data?.revenue ?? 0, data?.purchasing ?? 0)}%` }} />
          </div>
        ...
        <article className="rounded-2xl border border-slate-200/80 bg-white p-6">
          <div className="flex items-center justify-between"><div><p className="text-sm font-medium text-slate-500">Workforce today</p><h2 className="mt-1 text-xl font-semibold text-slate-900">Attendance pulse</h2></div><CalendarCheck className="size-5 text-[#3f6f9f]" /></div>
